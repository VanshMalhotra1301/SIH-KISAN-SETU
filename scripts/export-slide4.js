...
<body>
  <div class="slide-wrapper">
    <div class="slide-header">
      ...
      <div class="slide-title-main">FEASIBILITY AND VIABILITY</div>
      ${sihLogoSvg}
    </div>

    <div class="slide-subheader">
      <span class="icon">❖</span> Comprehensive Feasibility Analysis & Risk Mitigation
    </div>

    <div class="slide-content">
      <div class="card">
        <div class="card-header"><span style="color: #16a34a;">✅</span> Multidimensional Feasibility</div>
        <ul class="card-list">
          <li><strong>Technical Viability:</strong> Built on open-source, production-grade web stack (React 19, Supabase); zero proprietary hardware needed.</li>
          <li><strong>Data Integration:</strong> Seamlessly utilizes PM-KISAN database, Aadhaar KYC, State Land Records & CACP MSP rates.</li>
          <li><strong>Zero-Bar Adoption:</strong> Multilingual AI Sahayak (Hindi/Hinglish voice) eliminates digital literacy barriers for marginal farmers.</li>
          <li><strong>Infrastructure Cost:</strong> Cloud-native setup; mandis require only existing electronic weighbridges & standard smartphones.</li>
        </ul>
      </div>

      <div class="card">
        <div class="card-header"><span style="color: #0284c7;">🛡️</span> Challenges & Mitigation Strategies</div>
        <ul class="card-list">
          ...
          <li><strong>Scale to 7,000+ Mandis:</strong> Supabase RLS district partitioning + Vercel Edge CDN for high-throughput scaling.</li>
        </ul>
      </div>
    </div>
...
const tempHtml4 = path.resolve(outputDir, 'temp_slide4_exact.html');
fs.writeFileSync(tempHtml4, htmlSlide4);

const chromePath = fs.existsSync('C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe') 
  ? 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe'
  : 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe';

const targetImg4 = path.resolve(outputDir, 'sih_slide4_feasibility.jpg');
const brainImg4 = path.resolve(brainDir, 'sih_slide4_feasibility.jpg');

execSync(`"${chromePath}" --headless --disable-gpu --window-size=1280,720 --screenshot="${targetImg4}" "file:///${tempHtml4.replace(/\\\\/g, '/')}"`);
fs.copyFileSync(targetImg4, brainImg4);

console.log('Slide 4 with exact SIH logo re-exported!');
