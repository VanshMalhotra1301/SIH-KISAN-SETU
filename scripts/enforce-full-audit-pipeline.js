...
BEGIN;
  -- Try to add to publication, ignoring error if already added (Postgres doesn't have IF NOT EXISTS for this)
  DO $$
  BEGIN
    IF NOT EXISTS (
...
-- 2. Create Stored Procedure for Atomic Ticket Processing with Audit Logs
...
BEGIN
  ...
  IF p_action = 'call' THEN
    ...
    v_audit_metadata := jsonb_build_object('counter', coalesce(p_counter, 1));
  ...
  ELSIF p_action = 'weigh' THEN
    ...
    v_audit_metadata := jsonb_build_object(
      'gross_weight', p_gross,
      'tare_weight', p_tare,
      'net_quintals', coalesce(p_actual_quintals, p_gross - coalesce(p_tare, 0), v_ticket.quantity_quintals)
    );
  ...
  ELSIF p_action = 'grade' THEN
    ...
    v_audit_metadata := jsonb_build_object(
      'grade', coalesce(p_quality_grade, 'FAQ'),
      'moisture', coalesce(p_moisture, 11.5),
      'foreign_matter', coalesce(p_foreign_matter, 0.5)
    );
  ...
  ELSIF p_action = 'reject' THEN
    ...
    v_audit_metadata := jsonb_build_object('reason', coalesce(p_rejection_reason, 'Moisture / Foreign Matter exceeded standard limit'));
  ...
  ELSIF p_action = 'complete' OR p_action = 'accept' THEN
    ...
    v_audit_metadata := jsonb_build_object(
      ...
      'final_quintals', coalesce(p_actual_quintals, v_ticket.actual_quintals, v_ticket.quantity_quintals)
    );
  END IF;

  -- ENFORCE AUDIT PIPELINE: Log every action by the operator
  INSERT INTO public.audit_logs (actor_id, actor_role, action, target_type, target_id, metadata, created_at)
  VALUES (
    auth.uid(),
    'centre_operator',
    'operator_process_' || p_action,
    'queue_tickets',
    p_ticket_id,
    jsonb_build_object(
      'ticket_token', v_ticket.token,
      'centre_id', v_ticket.centre_id,
      'farmer_id', v_ticket.farmer_id,
      'details', coalesce(v_audit_metadata, '{}'::jsonb)
    ),
    now()
  );
...
async function run() {
  ...
  console.log("Upgrading Centre Operator schema for strict audit pipeline...");
  await client.query(schemaSql);
  console.log("✅ Schema upgraded and operator_process_ticket procedure updated with Audit Logging!");
  
  // Verify if it's in publication
  const pubRes = await client.query(`
    SELECT tablename FROM pg_publication_tables 
    WHERE pubname = 'supabase_realtime' AND tablename = 'audit_logs';
  `);
  if (pubRes.rows.length > 0) {
    console.log("✅ public.audit_logs is now successfully tracking via supabase_realtime publication!");
  } else {
    console.warn("⚠️ Warning: public.audit_logs not found in supabase_realtime. There might be an issue.");
  }
